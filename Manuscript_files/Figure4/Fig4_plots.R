...
data <- read.csv("HNC_HH_3examples.txt",sep="\t") 
...
ggplot(data2, aes(y = count, axis1 = lncRNA, axis2 = hallmark)) +
  ...
  theme(axis.text = element_text(size = 20)) #10 for top uTARs and width = 2/4


## choose top lncRNAs if required
top_lncRNAs <- data2 %>%
  group_by(lncRNA) %>%
...
## choose top hallmarks if required
top_hallmarks <- data2 %>%
  group_by(hallmark) %>%
  summarise(total_count = sum(count)) %>%
  top_n(8, total_count) %>%
...
data2h <- data2 %>%
  filter(hallmark %in% top_hallmarks)
